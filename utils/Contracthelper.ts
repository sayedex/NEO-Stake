...
export const getContractInstanceforhook = (
...
export const getNFTContractInstanceforhook = (
  contractaddress: string,
  account: any,
  library: any
) => {
...
export const ConvertEthTonormal = async (amount: any) => {
  const _amount = await ethers.utils.formatUnits(amount, 18);
  return _amount;
};
...
export function filterNFTsForStake(
  nftData: nftdata[],
  allowedAddress: any
): any[] {
  if (allowedAddress) {
    return nftData.filter(
      (nft) => nft.token_address.toLowerCase() == allowedAddress.toLowerCase()
    );
  }
  return [];
}
...
export function calculatePerDayReward(
...
export async function getListingDetails(
  ...
  const price = await ethers.utils.formatUnits(result[0], 18);
  return {
